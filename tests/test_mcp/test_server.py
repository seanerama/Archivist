...
class TestFormatDiff:
    def test_empty_diff(self) -> None:
        diff = DiffResult(family="nginx", from_version="1.22", to_version="1.24")
...
class TestToolInvocation:
    ...
    async def test_list_families_delegates(self) -> None:
        ...
        mock_families = [
            FamilyInfo(family_slug="nginx", doc_count=10, versions=["1.22", "1.24"], doc_types=["admin_guide"])
        ]
    ...
    async def test_version_diff_delegates(self) -> None:
        ...
        with patch("archivist.mcp.server.Retriever") as MockRetriever:
            mock_instance = MagicMock()
            mock_instance.version_diff.return_value = mock_diff
            MockRetriever.return_value = mock_instance
